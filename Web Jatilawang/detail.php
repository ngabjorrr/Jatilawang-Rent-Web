...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action_type === 'buy') {
        ...
        if (isset($_SESSION['unified_cart'][$item_key])) {
            $_SESSION['unified_cart'][$item_key]['quantity'] += $quantity;
            ...
            $_SESSION['unified_cart'][$item_key] = [
                ...
                'name' => $product['name'],
                'price' => $product['price'], // Harga satuan beli
                'quantity' => $quantity,
                'image' => $product['image'],
...
<div class="page-container">
    <div class="product-detail-container">
        ...
        <div class="product-detail-info-container">
            ...
            <p class="price">Rp <?= number_format($product['price'], 0, ',', '.') ?> <?php if ($product['is_rentable'] == 0 && $product['price'] > 0.00) { echo "(Harga Beli)"; } ?></p>
            ...
            <div class="product-actions" style="margin-top: 20px;">
                <?php
                $showBuyButton = false;
                if ($product['is_rentable'] == 0 && $product['price'] > 0.00) {
                    $showBuyButton = true;
                } elseif ($product['is_rentable'] == 1 && $product['price'] > 0.00) {
                    $showBuyButton = true;
                }
                ...
                ?>
                    <form method="post" action="detail.php?id=<?= htmlspecialchars($product['id']) ?>" style="margin-bottom: 15px;">
                        ...
                        <input type="number" id="quantity" name="quantity" value="1" min="1" style="padding: 8px; border: 1px solid #ccc; border-radius: 4px; width: 70px; margin-bottom: 10px; margin-right: 5px;">
                ...
                <?php elseif ($product['is_rentable'] == 0 && $product['price'] == 0.00) : ?>
                    <p><em>Produk ini tidak tersedia untuk dibeli saat ini.</em></p>
                <?php endif; ?>
                ...
                <?php if ($product['is_rentable'] == 1): ?>
                    <div class="rental-options" style="padding: 15px; border: 1px solid #007bff; border-radius: 5px; background-color: #f8f9fa; margin-top:15px;">
                        ...
                        <?php else: ?>
                            <p>Informasi harga sewa belum tersedia untuk produk ini atau harga sewa 0.</p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
